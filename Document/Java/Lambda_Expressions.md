...
>메소드를 참조해서 매개변수의 정보 및 리턴 타입을 알아내어 람다식에 필요한 매개변수를 제거하는 것이 목적
- **람다식이 메소드 참조로 대체된다.** 
- ex:) Math 클래스의 max() 정적메소드를 호출하는 람다식
```java
(left, right) -> Math.max(left, right);
//메소드의 매개값을 전달하는 역할만하기에  아래처럼 바꿔줄수 있다.
Math :: max;
//IntBinaryOperator 인터페이스는 두개의 int 매개값을 받아 int 값을 리턴하므로 Math::max 메소드참조를 대입할수 있다.
IntBinaryOperator operator = Math :: max;
```
- 메소드 참조는 정적 또는 인스턴스 메소드를 참조할 수 있고, 생성자 참조도 가능하다.

- **정적 메소드와 인스턴스 메소드 참조**
>정적(static) 메소드를 참조할 경우에는 클래스 이름뒤에 :: 기호를 붙익오 정적 메소드 이름을 기술한다.
```
클래스 :: 메소드
```
...
```
- **매개 변수의 메소드 참조**
>메소드가 람다식에서 제공되는 매개 변수의 멤버일 경우 
```java
//람다식에서 제공되는 a 매개변수의 메소드를 호출해서 b 매개변수를 매개값으로 사용하는 경우
(a,b) -> {a.instanceMethod(b); }
        ↓
클래스 :: instanceMethod
//정적 메소드 참조와 동일하지만 a의 인스턴스 메소드가 참조되므로 전혀 다른코드이다. 
```
- **생성자 참조**
>메소드 참조는 생성자 참조도 포함된다. 생성자를 참조한다는 것은 객체 생성을 의미한다. <br>단순히 객체를 생성하고 리턴하도록 구성된 람다식은 생성자 참조로 대체할 수 있다.
```java
(a,b) -> {return new 클래스(a,b); } 
        ↓
클래스 :: new
/*생성자가 오버로딩되어 여러개가 있을 경우, 컴파일러는 함수적 인터페이스의 추상 메소드와
 동일한 매개 변수 타입과 개수를 가지고 있는 생성자를 찾아 실행한다. */
```
