...
>서로 독립된 객체의 집합과 상호작용을 기반으로 프로그램을 설계하는 것
    ```
    //ex:)냉장고에 어제먹다 남긴 치킨을 꺼내는 상황을 프로그래밍
    1. 사람이라는 객체, 냉장고라는 객체, 치킨이라는 객체를 만든다.
    2. 사람이 냉장고에 무언가를 꺼내는 프로시저를 구현한다.
    3. 무언가의 위치에 치킨을 넣어 구현한 프로시저를 호출한다.
- 절차지향 (POP)
- 객체지향 (OOP)
## 캡슐화 (Encapsulation)
