...
public class BoardDAO {
	...
	public List<BoardVO> getBoardList(int page,int limit){
		...
		"(select * from smemberboard order by "+
	...
	public boolean boardInsert(BoardVO board){
		...
		try{
			...
			sql="insert into smemberboard (num,id,subject,";
			sql+="content,re_ref,re_lev,re_seq,readcount," +
	...
	public int boardReply(BoardVO board){
		...
		try{
			...
			sql="update smemberboard set re_seq=re_seq+1 ";
	...
	public boolean boardModify(BoardVO modifyboard) {
		String sql="update smemberboard set subject=?,";
